...
VOID TEST(SrsRtcConnectionTest, AddPlayerTypicalScenario)
{
    srs_error_t err;
    ...
    MockRtcAsyncTaskExecutor mock_exec;
    SrsContextId cid;
    cid.set_value("test-rtc-connection-add-player");
    ...
    // Create a mock RTC source that can publish (needed for play capability negotiation)
    SrsRtcSource* raw_source = new SrsRtcSource();
    mock_rtc_sources->mock_source_ = SrsSharedPtr<SrsRtcSource>(raw_source);
    ...
    // Create SrsRtcUserConfig for player
    SrsUniquePtr<SrsRtcUserConfig> ruc(new SrsRtcUserConfig());
    ruc->publish_ = false; // This is a player, not publisher
    ruc->audio_before_video_ = false;
    ...
    // Create a simple remote SDP that will fail negotiation (testing error handling)
    // This simulates a typical WebRTC player offer SDP
    ruc->remote_sdp_str_ = "v=0\r\n"
                          "o=- 123456 654321 IN IP4 127.0.0.1\r\n"
                          "s=-\r\n"
                          "t=0 0\r\n"
    ...
    // Test typical add_player scenario - expect it to fail due to SDP negotiation
    // This tests the error handling path of the add_player function
    HELPER_EXPECT_FAILED(conn->add_player(ruc.get(), local_sdp));

    // Verify that fetch_or_create was called during play capability negotiation
    EXPECT_EQ(1, mock_rtc_sources->fetch_or_create_count_);

    // The test verifies that:
    // 1. The function properly handles play capability negotiation
    // 2. Error propagation works correctly through the call stack
    // 3. The function fails gracefully when no play relations are found
    // 4. The RTC source manager is properly accessed during negotiation
}
